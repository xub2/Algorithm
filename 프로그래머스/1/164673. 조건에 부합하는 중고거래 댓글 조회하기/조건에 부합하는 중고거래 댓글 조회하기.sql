-- 코드를 입력하세요
--SELECT B.TITLE, B.BOARD_ID, R.REPLY_ID, R.WRITER_ID, R.CONTENTS, TO_CHAR(R.CREATED_DATE, 'YYYY-MM-DD') AS CREATED_DATE
--FROM USED_GOODS_BOARD B JOIN USED_GOODS_REPLY R ON B.BOARD_ID = R.BOARD_ID
--WHERE TO_CHAR(R.CREATED_DATE, 'YYYY-MM-DD') LIKE '2022-10%' -- String 비교니까...
--ORDER BY R.CREATED_DATE, B.TITLE

SELECT B.TITLE, B.BOARD_ID, R.REPLY_ID, R.WRITER_ID, R.CONTENTS, TO_CHAR(R.CREATED_DATE, 'YYYY-MM-DD') AS CREATE_DATE
FROM USED_GOODS_BOARD B
    JOIN USED_GOODS_REPLY R
        ON B.BOARD_ID = R.BOARD_ID
WHERE B.CREATED_DATE
    BETWEEN TO_DATE('2022-10-01', 'YYYY-MM-DD')
        AND TO_DATE('2022-10-31', 'YYYY-MM-DD')
ORDER BY R.CREATED_DATE ASC, B.TITLE ASC;